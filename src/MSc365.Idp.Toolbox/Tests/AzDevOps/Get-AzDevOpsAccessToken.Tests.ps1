BeforeAll {
    ...
    Mock Get-AzContext -ModuleName 'MSc365.Idp.Toolbox' -MockWith {
        return @{
            Tenant      = @{
    ...
    Mock Get-AzAccessToken -ModuleName 'MSc365.Idp.Toolbox' -MockWith {
        return @{
...
Describe 'Get-AzDevOpsAccessToken' {
    ...
    Context 'When retrieving access token with default tenant ID' {
        It 'Should retrieve access token using tenant ID from current Azure context' {
            # Act
            $result = Get-AzDevOpsAccessToken
            ...
            Should -Invoke Get-AzContext -ModuleName 'MSc365.Idp.Toolbox' -Exactly 1

            # Verify Get-AzAccessToken was called with correct parameters
            Should -Invoke Get-AzAccessToken -ModuleName 'MSc365.Idp.Toolbox' -Exactly 1 -ParameterFilter {
    ...
    Context 'When retrieving access token with specified tenant ID' {
        It 'Should retrieve access token using specified tenant ID' {
            ...
            # Act
            $result = Get-AzDevOpsAccessToken -TenantId $specifiedTenantId
            ...
            Should -Invoke Get-AzContext -ModuleName 'MSc365.Idp.Toolbox' -Exactly 0
        ...
        It 'Should accept empty string as tenant ID and use context tenant ID' {
            # Act
            $result = Get-AzDevOpsAccessToken -TenantId ''
    ...
    Context 'When Get-AzContext fails' {
        It 'Should throw an error when Get-AzContext fails' {
            ...
            # Act & Assert
            { Get-AzDevOpsAccessToken } | Should -Throw
        }
    ...
    Context 'When Get-AzAccessToken fails' {
        It 'Should throw an error when Get-AzAccessToken fails' {
            ...
            # Act & Assert
            { Get-AzDevOpsAccessToken } | Should -Throw 'Failed to get access token'
        }
    ...
    Context 'Parameter validation' {
        It 'Should accept valid GUID as TenantId' {
            ...
            # Act & Assert
            { Get-AzDevOpsAccessToken -TenantId $validGuid } | Should -Not -Throw
        }
        ...
        It 'Should accept string values for TenantId parameter' {
            ...
            # Act & Assert
            { Get-AzDevOpsAccessToken -TenantId $tenantIdString } | Should -Not -Throw
        }
    }
    ...
    Context 'Integration scenarios' {
        ...
        It 'Should handle multiple consecutive calls correctly' {
            # Act
            $result1 = Get-AzDevOpsAccessToken
            $result2 = Get-AzDevOpsAccessToken -TenantId '44444444-4444-4444-4444-444444444444'
            ...
            Should -Invoke Get-AzAccessToken -ModuleName 'MSc365.Idp.Toolbox' -Exactly 2
        }
    }
}
